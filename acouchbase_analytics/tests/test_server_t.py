...
class TestServerTestSuite:
    ...
    async def test_simple(self, test_env: AsyncTestEnvironment) -> None:
        ...
        rtask = test_env.cluster.execute_query(statement)
        print(f'Have result: {rtask=}')
        if isinstance(rtask, Task):
            print('Result is a Task')
            res = await rtask
