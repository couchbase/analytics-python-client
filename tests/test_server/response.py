...
class ServerResponseError:
    ...
    @staticmethod
    def _build_retry_group(retry_specification: NonRetriableSpecificationType,
                           err_count: int,
                           retriable_idx: Optional[int] = -1) -> List[ServerResponseError]:
    ...
    @staticmethod
    def build_retry_group(group_type: RetriableGroupType,
                          retry_specification: Optional[NonRetriableSpecificationType] = None,
                          err_count: Optional[int] = None) -> List[ServerResponseError]:
        ...
        if group_type == RetriableGroupType.Zero:
            ...
            return ServerResponseError._build_retry_group(retry_specification, err_count, retriable_idx=(err_count // 2))
    ...
    def build_errors(resp: ServerResponse,
                     ...
                     retry_specification: Optional[NonRetriableSpecificationType]=None,
                     err_count: Optional[int] = None) -> ServerResponse:
        if error_type == ErrorType.Timeout:
            ...
            resp.status = 'fatal'
            resp.metrics.error_count = 1
            resp.errors = [ServerResponseError(20001, 'Insufficient permissions or the requested object does not exist')]
